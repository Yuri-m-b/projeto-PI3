...
void *send_time_packet(void *arg) {
    ...
    while (1) {
        ...
        strftime(message, MESSAGE_SIZE, "Current Time: %Y-%m-%d %H:%M:%S", time_info); // Formata o horário em uma string

        // Envia o pacote UDP com a mensagem
        if (sendto(sock, message, strlen(message), 0, (const struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
            perror("Failed to send message");
        } else {
            printf("Sent: %s\n", message); // Exibe mensagem confirmando o envio
        }

        sleep(1); // Aguarda 1 segundo antes de enviar novamente
    }
