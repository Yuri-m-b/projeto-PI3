...
int main() {
    ...
    while (1) {
        ...
        if (len < 0) {
            perror("Receive failed"); // Exibe uma mensagem de erro caso a recepção falhe
        } else {
            buffer[len] = '\0'; // Adiciona o caractere nulo ao final da mensagem para garantir que seja uma string válida
            printf("Received: %s\n", buffer); // Exibe a mensagem recebida
        }
    }

    close(sock);  // Fecha o socket (esse trecho nunca será executado devido ao loop infinito)
    return 0;
}
